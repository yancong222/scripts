...
data = read.csv("alle2phaseExp.csv", header = TRUE)
summary(data)
############################accuracy check################################
...
byphase <- subset(data_sub, 
                  data_sub$trials_t1.ran == "1" | data_sub$trials_t2.ran == "1")
byphase$phase <- ifelse(byphase$trials_t1.ran == "1", "phase1", "phase2")
...
p + scale_x_discrete(name ="Competition group") +
